...
try: 
    from .tracker import TrackerBuilder
...
class Agent:
    ...
    def __init__(self, tracker, env = None, item = None):
        tb = TrackerBuilder()
        self.tracker = tb.get_tracker(tracker)
        self.name = tracker
        self.env = env
        self.item = item
...
class VerboseAgent(Agent):
    ...
    def __init__(self, tracker, env = None, item = None):
        print('Creating {} agent'.format(tracker))
        super().__init__(tracker, env, item)
        print('{} agent created'.format(tracker))
...
class TestAgent(VerboseAgent):
    ...
    def run(self):
        ...
        print('{} agent start run'.format(self.name))
        state = self.env.get_state() # color img from enviroment
